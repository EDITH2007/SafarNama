...
import { Compass, Mail, Lock, User, AlertCircle, Loader, ChevronRight } from "lucide-react";
  ...
  const handleOAuthSignUp = async () => {
    try {
      ...
      await withTimeout(
        ...
        15000,
        "Google Sign-Up is taking longer than expected. Please verify your connection or try again."
      );
    } catch (err: any) {
      console.error(err);
      setError(
        err.message && err.message.includes("taking longer")
          ? err.message
          : "Google Sign-Up failed. If you are running locally, please verify your redirect URIs or register with Gmail Credentials below."
      );
    } finally {
  ...
  const handleCredentialsSignUp = async (e: React.FormEvent) => {
    ...
    try {
      await withTimeout(
        ...
        15000,
        "Sign-up is taking longer than expected. Please verify your connection or try again."
      );
      ...
      if (err.message?.includes("taking longer")) {
        ...
        setError(err.message || "Failed to create profile. Email might already be registered.");
      }
    } finally {
      setLoading(false);
    }
